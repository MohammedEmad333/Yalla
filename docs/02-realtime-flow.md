...
السوكت يعمل والتطبيق مفتوح؛ أمّا لإيقاظ الكابتن/المستخدم والتطبيق **مغلق**
نستخدم Firebase Cloud Messaging كطبقة مكمّلة (لا بديلة):

```
حدث مهم (إسناد/تغيّر حالة/إلغاء)
   ├─ Socket.io  → للعملاء المتصلين الآن (لحظي، داخل التطبيق)
   └─ FCM Push   → لإيقاظ الجهاز حتى والتطبيق مغلق
```

- **تسجيل الرمز:** التطبيق يجلب `deviceToken` ويرسله عبر
  `POST /notifications/device-token` (يُحفظ في `deviceTokens[]` على User/Captain).
- **الإرسال:** `notification.service.js` يبني الحمولة (دوال نقيّة مُختبَرة) ويرسل
  عبر firebase-admin. **آمن:** إن لم تُضبط بيانات اعتماد Firebase
