...
> **الحالة:** كود الجوال مُدمج بالفعل (`mobile/lib/core/push/push_service.dart`،
> التهيئة في `main.dart`، الأذونات والقناة في `AndroidManifest.xml`)، والخادم يدعم
> `FCM_CREDENTIALS_JSON`. الخطوة الوحيدة المتبقّية على الجوال هي **الجزء 3.1**
> (`flutterfire configure`) لتوليد `lib/firebase_options.dart` بقيم مشروعك، ثمّ الالتزام به.
> بقيّة أكواد الأجزاء 3.4–3.6 أدناه مرجعيّة (مطبّقة فعلًا في المستودع).
