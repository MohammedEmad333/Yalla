...
async function upsertAdminMerchant(restaurantId, payload = {}) {
  ...
  if (userCollision || captainCollision) {
    throw httpError('رقم الجوال مستخدم في حساب آخر', 409);
  }
...
async function getProfile(merchantId) {
  ...
  if (!merchant || !merchant.isActive || !merchant.restaurant) {
    throw httpError('حساب المتجر غير متاح', 403);
  }
  return publicMerchant(merchant);
}
...
async function createMenuItem(merchantId, payload) {
  const merchant = await requireMerchant(merchantId);
  return restaurantService.createMenuItem(merchant.restaurant, payload);
}
...
async function updateMenuItem(merchantId, itemId, payload) {
  ...
  return restaurantService.updateMenuItem(itemId, payload);
}
...
async function listOrders(merchantId, query = {}) {
  const merchant = await requireMerchant(merchantId);
  const filter = {
    'store.restaurant': merchant.restaurant,
...
async function updateOrderStatus(merchantId, orderId, nextStatus) {
  ...
  if ([ORDER_STATUS.CANCELLED, ORDER_STATUS.DELIVERED].includes(order.status)) {
    throw httpError('لا يمكن تعديل طلب منتهٍ');
  }
  const current = order.store?.merchantStatus || 'new';
  if (!(MERCHANT_TRANSITIONS[current] || []).includes(nextStatus)) {
    throw httpError(`انتقال غير مسموح: ${current} -> ${nextStatus}`);
  }
  ...
  // تحديث كل الواجهات فورًا: تطبيق الشريك، الأدمن، الزبون، والكابتن إن وُجد.
  const socket = io.get();
  ...
  if (order.captain) {
    socket.to(ROOMS.captain(String(order.captain?._id || order.captain))).emit(EVENTS.ORDER_STATUS_UPDATED, order);
  }
  return order;
}
