...
async function login(phoneRaw, passwordRaw) {
  ...
  const merchant = await Merchant.findOne({ phone }).select('+passwordHash').populate('restaurant', 'name imageUrl active');
  if (merchant) {
    if (!(await merchant.verifyPassword(password))) throw httpError('كلمة السر غير صحيحة', 401);
    if (!merchant.isActive || !merchant.restaurant || merchant.restaurant.active === false) throw httpError('حساب المتجر غير متاح', 403);
    return {
      token: signToken(merchant._id, { restaurantId: String(merchant.restaurant._id), staffRole: 'owner' }),
      user: { id: merchant._id, name: merchant.name, phone: merchant.phone, role: ROLES.MERCHANT, staffRole: 'owner', restaurant: merchant.restaurant },
    };
  }

  const staff = await MerchantStaff.findOne({ phone }).select('+passwordHash').populate('restaurant', 'name imageUrl active');
...
async function list(merchantId) {
  return MerchantStaff.find({ merchant: merchantId }).select('-passwordHash').sort({ createdAt: 1 }).lean();
}
...
async function create(merchantId, auth, payload = {}) {
  ...
  await staff.save();
  return { id: staff._id, name: staff.name, phone: staff.phone, role: staff.role, active: staff.active };
}

async function update(merchantId, auth, staffId, payload = {}) {
  await requireOwner(merchantId, auth);
  const staff = await MerchantStaff.findOne({ _id: staffId, merchant: merchantId }).select('+passwordHash');
  if (!staff) throw httpError('الموظف غير موجود', 404);
...
async function remove(merchantId, auth, staffId) {
  await requireOwner(merchantId, auth);
  const row = await MerchantStaff.findOneAndDelete({ _id: staffId, merchant: merchantId });
  if (!row) throw httpError('الموظف غير موجود', 404);
  return { ok: true };
}
