...
class _RestaurantMenuScreenState extends State<RestaurantMenuScreen> {
  ...
  Future<void> _restoreCart() async {
    ...
    for (final section in _menu) {
      for (final item in section.items) { byId[item.id] = item; }
    }
    for (final raw in items) {
      ...
      if (variantName.isNotEmpty) {
        for (final v in item.variants) { if (v.label == variantName) { variant = v; break; } }
      ...
      for (final optionRaw in (row['options'] as List?) ?? const []) {
        ...
        for (final group in item.optionGroups.where((g) => g.name == groupName)) {
          for (final option in group.options) { if (option.name == optionName) { found = option; break; } }
        }
        if (found != null) selections.add(SelectedMenuOption(groupName, found.name, found.price));
      }
      final qty = ((row['qty'] as num?) ?? 1).toInt().clamp(1, 50);
      for (var i = 0; i < qty; i++) { _cart.add(item, variant: variant, options: selections); }
    }
  }

  Future<void> _persistCart() => CartStorage.save(restaurantId: _restaurant.id, items: _cart.toItemsPayload());
  ...
  Future<void> _rate() async {
    final stars = await showDialog<int>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Row(mainAxisAlignment: MainAxisAlignment.center, children: List.generate(5, (i) => IconButton(
    ...
    try { await _repo.rate(_restaurant.id, stars); await _load(); }
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    body: _loading
        ...
        : _error.isNotEmpty
            ? Center(child: Column(mainAxisSize: MainAxisSize.min, children: [Text(_error), const SizedBox(height: 12), FilledButton(onPressed: _load, child: const Text('إعادة المحاولة'))]))
  ...
  Widget _content() => ListView(
    ...
    children: [
      if (_restaurant.fullImageUrl != null)
        CachedNetworkImage(imageUrl: _restaurant.fullImageUrl!, height: 210, width: double.infinity, fit: BoxFit.cover, placeholder: (_, __) => const SizedBox(height: 210, child: Center(child: CircularProgressIndicator())), errorWidget: (_, __, ___) => _imageFallback())
      else _imageFallback(),
      Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            ...
            FilledButton.tonalIcon(onPressed: _rating ? null : _rate, icon: const Icon(Icons.star_outline), label: const Text('تقييم')),
          ]),
          const SizedBox(height: 6),
          Text(_restaurant.ratingCount > 0 ? '⭐ ${_restaurant.ratingAverage.toStringAsFixed(1)} (${_restaurant.ratingCount})' : 'لا توجد تقييمات بعد', style: TextStyle(color: YallaColors.muted)),
          if (_restaurant.description.isNotEmpty) ...[const SizedBox(height: 8), Text(_restaurant.description)],
          ...
          Wrap(spacing: 12, runSpacing: 8, children: [
            ...
            if (!_cart.isEmpty) const Chip(avatar: Icon(Icons.save_outlined, size: 16), label: Text('السلة محفوظة')),
          ]),
        ]),
      ),
      if (_menu.isEmpty) const Padding(padding: EdgeInsets.all(36), child: Center(child: Text('لا توجد أصناف بعد'))),
      for (final section in _menu) ...[
        Padding(padding: const EdgeInsets.fromLTRB(16, 18, 16, 8), child: Text(section.category, style: const TextStyle(fontSize: 19, fontWeight: FontWeight.w900))),
        ...section.items.map(_itemTile),
      ],
    ],
  );

  Widget _imageFallback() => Container(height: 210, color: YallaColors.surfaceContainer, child: const Center(child: Icon(Icons.storefront, size: 64)));
        ...
        child: Padding(
          ...
          child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
            if (item.fullImageUrl != null)
              ClipRRect(borderRadius: BorderRadius.circular(12), child: CachedNetworkImage(imageUrl: item.fullImageUrl!, width: 82, height: 82, fit: BoxFit.cover))
            else Container(width: 82, height: 82, decoration: BoxDecoration(color: YallaColors.surfaceContainer, borderRadius: BorderRadius.circular(12)), child: const Icon(Icons.fastfood_outlined)),
            const SizedBox(width: 12),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(item.name, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w800)),
              if (item.description.isNotEmpty) ...[const SizedBox(height: 4), Text(item.description, maxLines: 2, overflow: TextOverflow.ellipsis, style: TextStyle(color: YallaColors.muted, fontSize: 13))],
              const SizedBox(height: 8),
  ...
  Widget _cartBar() => SafeArea(
    ...
    child: Padding(
      ...
      child: FilledButton(
        ...
        child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Badge(label: Text('${_cart.count}')), const Text('عرض السلة'), Text('${_cart.total} ₪')]),
      ),
    ),
  );
}
...
class _ItemOptionsSheetState extends State<_ItemOptionsSheet> {
  ...
  void initState() {
    ...
    for (final g in widget.item.optionGroups) { _selected[g.name] = <String>{}; }
  }
  ...
  void _toggle(MenuOptionGroup group, MenuOption option) {
    ...
    setState(() {
      _error = '';
      if (group.multiple) {
        if (set.contains(option.name)) set.remove(option.name);
        else if (set.length < group.maxSelect) set.add(option.name);
      } else {
        set..clear()..add(option.name);
      }
    });
  }
  ...
  void _done() {
    ...
    for (final group in widget.item.optionGroups) {
      ...
      if (names.length < min) { setState(() => _error = 'اختر ${group.name}'); return; }
      ...
      child: SingleChildScrollView(
        child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          ...
          if (widget.item.variants.isNotEmpty) ...[
            ...
            const SizedBox(height: 6),
            ...widget.item.variants.map((v) => RadioListTile<MenuVariant>(value: v, groupValue: _variant, onChanged: (x) => setState(() => _variant = x), title: Text(v.label), secondary: Text('${v.price} ₪'))),
          ],
          for (final group in widget.item.optionGroups) ...[
            ...
            ...group.options.map((o) => CheckboxListTile(value: (_selected[group.name] ?? {}).contains(o.name), onChanged: (_) => _toggle(group, o), title: Text(o.name), secondary: o.price > 0 ? Text('+${o.price} ₪') : null)),
          ],
